...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try { 
        ...
        if ($fileCount > MAX_UPLOAD_FILES) {
            throw new Exception("Too many files. Maximum " . MAX_UPLOAD_FILES . " photos allowed.");
        }
        ...
        for ($x = 0; $x < $fileCount; $x++) {
            ...
            if ($fsize > $limitPerFileBytes) {
                ...
                throw new Exception("File too large (" . MAX_FILE_SIZE_MB . "MB max): {$name}");
            }

            if ($totalSize + $fsize > $totalLimitBytes) {
                throw new Exception("Total upload size exceeds " . MAX_TOTAL_SIZE_MB . "MB limit. Please reduce file count or sizes.");
            }
        ...
        for ($x = 0; $x < $fileCount; $x++) {
            ...
            if (!is_uploaded_file($ftemp)) {
                throw new Exception("Invalid upload source: " . htmlspecialchars($originalName));
            }
            ...
            if (!in_array($mime, $allowedMimes, true)) {
                throw new Exception("Unsupported image type: " . htmlspecialchars($originalName));
            }
            ...
            if (file_exists($destPath)) {
                throw new Exception("Failed to upload {$originalName}: file already exists.");
            }
            ...
            if ($fsize <= 5 * 1024 * 1024) {
                ...
                if ($img) {
                    ...
                    throw new Exception("Failed to create image: " . htmlspecialchars($originalName));
                }
            }

            if (!$ok) {
                throw new Exception("Failed to save: " . htmlspecialchars($originalName));
            }
