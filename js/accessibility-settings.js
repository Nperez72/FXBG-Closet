...
class AccessibilitySettings {
  ...
  applySettings() {
    ...
    // Apply font family
    const fontFamily =
      this.fontFamilies[this.settings.fontFamily]?.value ||
      this.fontFamilies.quicksand.value;
    ...
    // Apply font weight
    const fontWeight =
      this.fontWeights[this.settings.fontWeight]?.value || "400";
    root.style.setProperty("--accessibility-font-weight", fontWeight);

    // Apply line height
    const lineHeight =
      this.lineHeights[this.settings.lineHeight]?.value || "1.5";
    root.style.setProperty("--accessibility-line-height", lineHeight);

    // Apply letter spacing
    const letterSpacing =
      this.letterSpacings[this.settings.letterSpacing]?.value || "0";
    root.style.setProperty("--accessibility-letter-spacing", letterSpacing);

    // Apply text align
    const textAlign =
      this.textAligns[this.settings.textAlign]?.value || "inherit";
  ...
  updatePreview() {
    ...
    if (preview) {
      ...
      const letterSpacing =
        this.letterSpacings[this.settings.letterSpacing]?.value;
